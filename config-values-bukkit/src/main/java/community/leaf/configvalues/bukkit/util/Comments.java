...
public class Comments
{
    ...
    public static boolean isSupported() { return SUPPORTS_COMMENTS; }
    ...
    {
        if (SUPPORTS_COMMENTS) { section.setComments(key, lines); }
    }
    ...
    {
        if (SUPPORTS_COMMENTS) { section.setInlineComments(key, lines); }
    }
    ...
    {
        if (SUPPORTS_COMMENTS) { return config.options().getHeader(); }
    ...
    {
        if (SUPPORTS_COMMENTS) { config.options().setHeader(lines); }
        else
        {
            if (lines == null || lines.isEmpty()) { config.options().header(null); }
            else
            {
    ...
    {
        if (SUPPORTS_COMMENTS) { config.options().setFooter(lines); }
    }
}
